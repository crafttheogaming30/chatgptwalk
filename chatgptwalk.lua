--[[ 
 AUTO WALK TRACK SYSTEM - HP READY FINAL v8
 Theme : Dark Blue
 UI     : Scrollable, Draggable, Modular, History, Speed Control + Animasi
...
nameBox.Size = UDim2.fromOffset(280,30)
...
sliderBg.Size = UDim2.fromOffset(280,8)
sliderBg.Position = UDim2.fromOffset(20,y+40)
sliderBg.BackgroundColor3 = THEME.header
Instance.new("UICorner", sliderBg).CornerRadius = UDim.new(0,4)
local knob = Instance.new("Frame", sliderBg)
knob.Size = UDim2.fromOffset(14,14)
knob.Position = UDim2.new(speed/50, -7, -0.25,0)
knob.BackgroundColor3 = THEME.accent
Instance.new("UICorner", knob).CornerRadius = UDim.new(1,0)

local plusBtn = addButton(body,"+",y+60)
plusBtn.Size = UDim2.fromOffset(40,24)
plusBtn.Position = UDim2.fromOffset(200,y+60)
plusBtn.MouseButton1Click:Connect(function() speed = math.clamp(speed+1,1,50); knob.Position = UDim2.new(speed/50,-7,-0.25,0); toast("Speed: "..speed.."x") end)
local minusBtn = addButton(body,"-",y+60)
minusBtn.Size = UDim2.fromOffset(40,24)
minusBtn.Position = UDim2.fromOffset(150,y+60)
minusBtn.MouseButton1Click:Connect(function() speed = math.clamp(speed-1,1,50); knob.Position = UDim2.new(speed/50,-7,-0.25,0); toast("Speed: "..speed.."x") end)
...
knob.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then
        ...
        moveConn = RunService.RenderStepped:Connect(function()
            ...
            knob.Position = UDim2.new(x,-7,-0.25,0)
            speed = math.floor(1 + x*49)
        end)
        i.Changed:Connect(function()
            if i.UserInputState==Enum.UserInputState.End then moveConn:Disconnect(); toast("Speed: "..speed.."x") end
        end)
    end
end)
...
recordData, playing, loopTrack, paused = {}, false,false,false
...
recordBtn.MouseButton1Click:Connect(function()
    if recording then
        ...
        -- otomatis simpan ke histori
        if #recordData>0 then
            local fname=(nameBox.Text~="" and nameBox.Text or "Track")..".lua"
            local data="return {"
            for _,p in ipairs(recordData) do data..=string.format("{Position=Vector3.new(%f,%f,%f),Anim='%s'},",p.X,p.Y,p.Z,'') end
            data..="}"
            writefile("tracks/"..fname,data)
            toast("Track tersimpan ke histori")
        end
        return
    end
    recording=true; paused=false; recordData={}
    recordConn=RunService.RenderStepped:Connect(function()
        if recording and not paused then
            table.insert(recordData,{Position=hrp.Position,Anim='Idle'}) -- placeholder anim
        end
    end)
